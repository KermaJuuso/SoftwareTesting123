...
describe("filter()", () => {
    ...
    it('should return an empty array', () => {
        const productsArray = Object.values(testProducts);
        const result = filter(productsArray, product => product.category.includes("ASDSA"));
        expect(result).to.be.an('array').that.has.lengthOf(0);
        
    });

    it('null as input should return an empty array', () => {
        const result = filter(null, product => product.category.includes("ASDSA"));
        expect(result).to.be.an('array').that.has.lengthOf(0);

    });

    it('undefined as input should return an empty array', () => {
        const result = filter(undefined, product => product.category.includes("ASDSA"));
        expect(result).to.be.an('array').that.has.lengthOf(0);
    });

    it('String as value should return TypeError', () => {
        const text = "THIS IS EPIC"
        expect(() => filter(text, product => product.category.includes("ASDSA"))).to.throw(TypeError);
    });
    ...
    it('Product with undefined name should throw typeError', () => {
        const productsArray = Object.values(testProducts);
        expect(() => filter(productsArray, product => product.name.includes("Think"))).to.throw(TypeError);
    });
